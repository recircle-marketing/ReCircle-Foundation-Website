/**
 * Seed sample blogs for the Knowledge Centre.
 *
 * Run:
 *   node /app/backend/src/scripts/seedBlogs.js
 *
 * Idempotent — skips blogs whose slug already exists.
 */
...
const SAMPLE_BLOGS = [
  {
    ...
    summary:
      "India's recycling system runs on the labour of waste workers who remain invisible to the formal economy. Recognising and protecting them is the first step toward a truly inclusive circular future.",
    featured_image:
      'https://recircle.in/wp-content/uploads/2026/03/FORMALISATION-OF-SAFAI-SAATHIS-1.png',
  ...
  },
  {
    ...
    summary:
      'Every kilo of waste diverted from a landfill is a small but real climate intervention. Here is how organised recovery systems compound into measurable environmental outcomes.',
  ...
  },
  {
    ...
    summary:
      'We spent six months working with residents in Mumbai. The biggest unlock for circularity was not technology — it was a clearer label, a friendlier conversation, and a small daily ritual.',
  ...
  },
  {
    ...
    summary:
      'Traceability turns waste recovery from a black box into a trusted supply chain. It is also the single biggest enabler of corporate accountability under EPR.',
  ...
  },
  {
    ...
    summary:
      'Healthcare for Safai Saathis is not a perk—it is the floor on which everything else stands. We share what we have learned from running mobile health camps across recovery sites.',
...
async function run() {
  ...
  for (const b of SAMPLE_BLOGS) {
    ...
    const existing = await db.collection('blogs').findOne({ slug }, { projection: { id: 1 } });
    if (existing) {
      skipped += 1;
      continue;
    }
    const date = new Date();
    date.setDate(date.getDate() - b.days_ago);
